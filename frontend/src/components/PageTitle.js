...
function PageTitle()
{
    ...
    return(
        <nav className="navbar navbar-expand-lg navbar-warning bg-warning">
            ...
            {isLogIn ? <button type="button" id="logOutButton" className="btn btn-outline-danger font-weight-bold" onClick={doLogout}>Sign Out</button> : <Link to="/signup" className="btn btn-outline-alert font-weight-bold">Sign up</Link>}
            </div>
        </nav> 
    );
};
